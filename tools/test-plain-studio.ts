...
async function main() {
  ...
  // fader = the lane's Excel dB; clips input-normalized to (dB − base)
  assert(close(ss1.volume, 0.501, 0.002), `SS-1 fader −6 dB (0.501) — got ${ss1.volume}`)
  assert(ss1.clips.filter((c) => c.calibrateDb === 0).length === 2 && ss1.clips.some((c) => c.calibrateDb === -14), `SS-1 normalization targets 0/0/−14 vs the fader`)
  ...
  // MUS-1 base −6 → fader 0.501; F1–F2 clips normalized −12 vs fader
  assert(close(mus.volume, 0.501, 0.002) && mus.clips.filter((c) => c.calibrateDb === -12).length === 2 && mus.clips.filter((c) => c.calibrateDb === 0).length === 4, `MUS-1 fader −6 dB, targets −12×2 + 0×4`)
  ...
  assert(close(bin.volume, 0.355, 0.002) && bin.clips[0].calibrateDb === 0 && bin.clips[1].calibrateDb === -9, `BIN-1 fader −9 dB, targets 0 / −9`)
  ...
  assert(close(guide.volume, 1.0, 0.002) && guide.clips.every((c) => c.calibrateDb === 0), `VOX-C fader 0.0 dB (gain 1.0), clips normalized to the anchor`)
  ...
  assert(close(right.volume, 0.501, 0.002) && right.clips.every((c) => c.calibrateDb === 0), `VOX-R fader −6 dB, clips normalized to the anchor`)
