...
export function PatientEdit({ patient, onCancel, onSave }: Props) {
  ...
  return (
    <div className="b2b-page">
      ...
      <p className="b2b-sub" style={{ marginBottom: 20 }}>{tr('Changes are saved to the patient\'s record.')}</p>

      <div className="pe-grid">
        <section className="b2b-card">
          <h2 className="b2b-card__title">{tr('Identity')}</h2>
          <label className="pe-field"><span className="pe-label">{tr('Name')}</span>
            <input className="b2b-input" value={name} onChange={(e) => setName(e.target.value)} /></label>
          <div className="pe-row">
            <label className="pe-field"><span className="pe-label">{tr('Age')}</span>
              <input className="b2b-input" type="number" value={age} onChange={(e) => setAge(e.target.value)} /></label>
            <label className="pe-field pe-field--grow"><span className="pe-label">{tr('Reason for care')}</span>
              <input className="b2b-input" value={reason} onChange={(e) => setReason(e.target.value)} /></label>
          </div>
        </section>

        <section className="b2b-card">
          <h2 className="b2b-card__title">{tr('Clinical')}</h2>
          <label className="pe-field"><span className="pe-label">Conditions <em>{tr('comma-separated')}</em></span>
            <input className="b2b-input" value={conditions} onChange={(e) => setConditions(e.target.value)} /></label>
          <label className="pe-field"><span className="pe-label">Medications <em>{tr('comma-separated')}</em></span>
            <input className="b2b-input" value={medications} onChange={(e) => setMedications(e.target.value)} /></label>
          <label className="pe-field"><span className="pe-label">{tr('Prescription')}</span>
            <input className="b2b-input" value={prescription} onChange={(e) => setPrescription(e.target.value)} placeholder={tr('e.g. 3× GL-ANX Quick / week')} /></label>
        </section>

        <section className="b2b-card">
          <h2 className="b2b-card__title">{tr('Goals')}</h2>
          <ul className="pe-goals">
            {goals.map((g, i) => (
              <li key={i} className="pe-goal">
                <input className="b2b-input" value={g.text} placeholder={tr('Goal')} onChange={(e) => setGoal(i, { text: e.target.value })} />
                <select className="pe-select" value={g.status} onChange={(e) => setGoal(i, { status: e.target.value as Goal['status'] })}>
                  {STATUSES.map((s) => <option key={s} value={s}>{s.replace('-', ' ')}</option>)}
                </select>
                <button className="pe-del" onClick={() => setGoals((gs) => gs.filter((_, idx) => idx !== i))} aria-label={tr('Remove goal')}>✕</button>
              </li>
            ))}
          </ul>
          <button className="pe-add" onClick={() => setGoals((gs) => [...gs, { text: '', status: 'in-progress' }])}>{tr('+ Add goal')}</button>
        </section>

        <section className="b2b-card">
          <h2 className="b2b-card__title">{tr('Next appointment')}</h2>
          <label className="pe-field"><span className="pe-label">{tr('Date & time')}</span>
        ...
        <section className="b2b-card pe-card--wide">
          <h2 className="b2b-card__title">{tr('Clinical notes')} <span className="lock">🔒 {tr('therapist only')}</span></h2>
          <textarea className="pe-notes" rows={4} value={notes} onChange={(e) => setNotes(e.target.value)} />
        </section>
      </div>

      <div className="pe-actions">
        <button className="b2b-btn b2b-btn--ghost" onClick={onCancel}>{tr('Cancel')}</button>
        <button className="b2b-btn b2b-btn--primary" disabled={busy} onClick={save}>{busy ? tr('Saving…') : tr('Save changes')}</button>
      </div>
    </div>
  )
}
