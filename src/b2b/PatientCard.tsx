    ...
    <div className="b2b-page">
      ...
      <div className="card-head">
        <div className="card-head__id">
          ...
          <div>
            ...
            <p className="b2b-sub">{p.age} · {p.sex === 'F' ? tr('Female') : tr('Male')} · {p.reason}</p>
          </div>
        </div>
        <div className="card-head__cta">
          {p.nextSessionAt && <span className="b2b-sub">Next: {relWhen(p.nextSessionAt)}</span>}
          <button className="b2b-btn b2b-btn--ghost" onClick={onEdit}>{tr('Edit record')}</button>
          <button className="b2b-btn b2b-btn--primary" onClick={onStartSession}>{tr('Start session →')}</button>
        </div>
      </div>

      {/* continuity briefing (auto) */}
      <div className="continuity">
        <span className="continuity__tag">{tr('Pre-session briefing')}</span>
        <span>
          {sinceLast.length
      ...
      <div className="card-grid">
        ...
        <section className="b2b-card">
          <h2 className="b2b-card__title">{tr('Clinical snapshot')}</h2>
          <dl className="kv">
            <dt>{tr('Active conditions')}</dt><dd>{p.conditions.join(', ') || '—'}</dd>
            <dt>{tr('Medications')}</dt><dd>{p.medications.join(', ') || 'None'}</dd>
            <dt>{tr('GL contraindications')}</dt><dd>{p.contraindications.join(', ')}</dd>
            <dt>{tr('Prescription')}</dt><dd>{p.prescription ?? 'None set'}</dd>
          </dl>
        </section>

        {/* scores */}
        <section className="b2b-card">
          <h2 className="b2b-card__title">{tr('Assessment trend (T0 → T2)')}</h2>
        ...
        {/* B2B chronology */}
        <section className="b2b-card">
          <h2 className="b2b-card__title">{tr('Session history')}</h2>
          <ul className="chron">
            {p.b2bSessions.length === 0 && <li className="b2b-sub">{tr('No sessions yet')}</li>}
        ...
        <section className="b2b-card">
          <h2 className="b2b-card__title">B2C self-practice {linked && <span className="link-badge">{tr('● linked account · live')}</span>}</h2>
          <ul className="chron">
            {p.b2cSessions.length === 0 && <li className="b2b-sub">{tr('No self-practice logged')}</li>}
        ...
        <section className="b2b-card">
          ...
          {p.messages.length > 0 && (
            <>
              <h3 className="b2b-card__sub">{tr('Messages')}</h3>
              <ul className="msgs">
                {p.messages.map((m, i) => (
